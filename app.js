...
app.delete('/data/${year}', (req, res) => {
    fs.readFile(graphData, 'utf8', (err, data) => {
      var existItems = getAccountData()
      const itemYear = req.params['year'];
      delete existAccounts[itemYear]; 
      saveAccountData(existItems);
      res.send(`item with id ${itemYear} has been deleted`)
    }, true);
  })